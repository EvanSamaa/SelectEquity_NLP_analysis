...
def news_search(query_terms, savepath, ealiest_date="2000-01-01", latest_date=""):
    ...
    for item in os.listdir("data/"):
        print(item)
        if item != "covid.csv" and item != "covid_new_keyword.csv" and item != "testdata" and item != "training.1600000.processed.noemoticon.csv":
            print(item)
            full_path = "data/" + item
...
if __name__ == "__main__":
    term = "covid OR SARS OR coronavirus OR cov19 OR pandemic OR novel virus OR covid-19 OR Wuhan Virus OR corona virus"
    path = "./data/covid_by_keyword.csv"
